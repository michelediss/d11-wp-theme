...
- `D11 Engine` should read `docs/` when theme documentation is required, while page workflow skills provided by the separate private `d11-engine-figma` repository must still derive runtime facts for blocks, tokens, screenshots, and audits from concrete theme code plus workflow artifacts.
