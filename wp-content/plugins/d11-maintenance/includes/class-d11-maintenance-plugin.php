...
final class D11_Maintenance_Plugin
{
    ...
    public const COOKIE_NAME = 'd11_maintenance_bypass';
    public const SETTINGS_SLUG = 'd11-maintenance';
    public const ACTION_SAVE = 'd11_maintenance_save_settings';
    public const ACTION_REGENERATE = 'd11_maintenance_regenerate_bypass';
    public const NOTICE_ARG = 'd11_maintenance_notice';
    ...
    {
        add_options_page(
            __('Maintenance', 'd11-maintenance'),
            __('Maintenance', 'd11-maintenance'),
            'manage_options',
            self::SETTINGS_SLUG,
    ...
    public function handle_settings_save(): void
    {
        if (! current_user_can('manage_options')) {
            wp_die(esc_html__('You are not allowed to manage maintenance settings.', 'd11-maintenance'));
        }

        check_admin_referer(self::ACTION_SAVE);

        $raw_settings = wp_unslash($_POST['d11_maintenance'] ?? []);
    ...
    public function render_settings_page(): void
    {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Maintenance', 'd11-maintenance'); ?></h1>
            <p><?php esc_html_e('Enable a site-wide maintenance response powered by the Gutenberg content of the fixed Maintenance page.', 'd11-maintenance'); ?></p>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <table class="form-table" role="presentation">
                    <tbody>
                        <tr>
                            <th scope="row"><?php esc_html_e('Status', 'd11-maintenance'); ?></th>
                            <td>
                                <label for="d11-maintenance-enabled">
                                    <input
                                        id="d11-maintenance-enabled"
                                        type="checkbox"
                                        name="d11_maintenance[enabled]"
                                        value="1"
                                        <?php checked($settings['enabled']); ?>
                                    >
                                    <?php esc_html_e('Enable maintenance mode', 'd11-maintenance'); ?>
                                </label>
                                <p class="description">
                                    <?php esc_html_e('When enabled, public frontend HTML requests are replaced with the Maintenance page and return HTTP 503.', 'd11-maintenance'); ?>
                                </p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Source page', 'd11-maintenance'); ?></th>
                            <td>
                                ...
                                <?php else : ?>
                                    <p class="description"><?php esc_html_e('The required page was not found or is not published.', 'd11-maintenance'); ?></p>
                                <?php endif; ?>
                                <p class="description"><?php esc_html_e('The plugin always uses the page with slug Maintenance / maintenance. No page selector is exposed here.', 'd11-maintenance'); ?></p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><?php esc_html_e('Bypass link', 'd11-maintenance'); ?></th>
                            <td>
                                <input
                                    id="d11-maintenance-bypass-link"
                                    type="text"
                                ...
                                >
                                <p>
                                    <button type="button" class="button" data-copy-target="d11-maintenance-bypass-link"><?php esc_html_e('Copy link', 'd11-maintenance'); ?></button>
                                </p>
                                <p class="description">
                                    <?php
                                    printf(
                                        /* translators: %d: number of days. */
                                        esc_html__('Visitors using this link receive a bypass cookie valid for %d days.', 'd11-maintenance'),
                        ...
                        <tr>
                            <th scope="row"><?php esc_html_e('Automatic access', 'd11-maintenance'); ?></th>
                            <td>
                                <p><?php esc_html_e('Administrators bypass maintenance automatically when logged in. External users can access the site only through the bypass link.', 'd11-maintenance'); ?></p>
                            </td>
                        </tr>
                    </tbody>
                </table>

                <?php submit_button(__('Save Maintenance Settings', 'd11-maintenance')); ?>
            </form>
            ...
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin-top: 1rem;">
                ...
                <?php submit_button(__('Regenerate bypass token', 'd11-maintenance'), 'secondary', 'submit', false); ?>
            </form>
        </div>
    ...
    private function render_admin_notice(string $notice, ?WP_Post $maintenance_page): void
    {
        if ($notice === self::NOTICE_SAVED) {
            $this->render_notice_message(__('Maintenance settings saved.', 'd11-maintenance'), 'updated');
        }

        if ($notice === self::NOTICE_REGENERATED) {
            $this->render_notice_message(__('Bypass token regenerated.', 'd11-maintenance'), 'updated');
        }

        if (! $maintenance_page instanceof WP_Post) {
            $this->render_notice_message(
                __('The required page with slug "maintenance" is missing or not published. Maintenance mode will not block the site until that page exists.', 'd11-maintenance'),
                'error'
            );
        }
    }
