...
const READY_ATTR = 'data-d11-privacy-motion-ready';
...
function getBannerElements() {
  const banner = document.getElementById('d11-privacy-banner');
  ...
  return {
    ...
    settings: banner.querySelector('#d11-privacy-settings'),
...
export function initD11PrivacyBanner() {
  ...
  document.addEventListener('d11-privacy:show', (event) => {
    const detail = event.detail || {};
    animateBannerIn(getBannerElements(), !!detail.openSettings);
  });

  document.addEventListener('d11-privacy:before-hide', (event) => {
    const detail = event.detail || {};
  ...
  document.addEventListener('d11-privacy:settings-change', (event) => {
    const detail = event.detail || {};
