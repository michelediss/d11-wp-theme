...
2. Let `optimize-lighthouse` run its environment preflight before the audit.
3. In the `d11` workspace, prefer the automatic D11-first runner selection:
   - use the canonical site URL when it is reachable from the current host
   - otherwise let the CLI delegate to the internal Docker audit runner automatically
4. Run `optimize-lighthouse` with the requested device and run count.
5. Inspect the filtered metrics, normalized opportunities, prioritized issues, and execution metadata.
6. If a baseline JSON exists, run compare mode and inspect regressions first.
7. Produce or apply concrete frontend fixes that are safe within the current theme architecture.
8. Re-run the audit when verification is needed after changes.

## Report retention

- The default `dev-lighthouse/<page-name>/` directory is treated as a replaceable artifact location, not as a historical archive.
- Before each new run, the CLI removes the existing report files for that page and writes only the reports generated by the current run.
- Do not keep a compare baseline inside the same page directory when using `--compare`, because the cleanup phase removes that directory before the new run.
...
- runner type plus canonical URL vs audit URL when they differ
