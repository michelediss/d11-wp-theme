(function (window, document, wp) {
  ...
  const config = window.D11Privacy || {};
  ...
  function getBanner() {
    return document.getElementById("d11-privacy-banner");
  }

  function getSettingsPanel() {
    return document.getElementById("d11-privacy-settings");
  }
  ...
  function setSettingsState(open) {
    ...
    dispatchBannerEvent("d11-privacy:settings-change", {
      banner: banner,
      settings: settings,
      open: !!open
    });
  ...
  function showBanner(openSettings) {
    ...
    banner.focus();
    dispatchBannerEvent("d11-privacy:show", {
      banner: banner,
  ...
  function hideBanner(reason) {
    ...
    const event = dispatchBannerEvent("d11-privacy:before-hide", {
      banner: banner,
      settings: settings,
  ...
  function syncToggles() {
    ...
    defaultCategories.forEach(function (category) {
      const input = banner.querySelector('[data-d11-privacy-toggle="' + category + '"]');
      if (input) {
  ...
  function setCookiePanelState(category, open) {
    const panel = document.querySelector('[data-d11-privacy-cookie-panel="' + category + '"]');
    const toggle = document.querySelector('[data-d11-privacy-cookie-toggle="' + category + '"]');
  ...
  function renderCookieLists() {
    ...
    const lists = document.querySelectorAll("[data-d11-privacy-cookie-list]");
    const serviceLabel = escapeHTML(__("Service", "d11"));
    const durationLabel = escapeHTML(__("Duration", "d11"));
    const descriptionLabel = escapeHTML(__("Description", "d11"));

    lists.forEach(function (list) {
      const category = list.getAttribute("data-d11-privacy-cookie-list");
      const items = details[category] || [];
      const row = document.querySelector('[data-d11-privacy-cookie-row="' + category + '"]');
      const panel = document.querySelector('[data-d11-privacy-cookie-panel="' + category + '"]');
  ...
  function activateConsentEmbeds() {
    ...
    document.querySelectorAll(".d11-privacy-embed[data-d11-privacy-embed-html]").forEach(function (wrap) {
      if (wrap.dataset.simpleCookieConsentLoaded === "1") {
        return;
      }

      const body = wrap.querySelector(".d11-privacy-embed__body");
      const encoded = wrap.getAttribute("data-d11-privacy-embed-html");
      const placeholder = wrap.querySelector(".d11-privacy-embed__placeholder");
  ...
  function blockEmbedsIfNeeded() {
    if (hasConsent("marketing")) {
      document.querySelectorAll("iframe[data-d11-privacy-blocked='1']").forEach(function (iframe) {
        const originalSrc = iframe.getAttribute("data-d11-privacy-original-src");
        ...
        iframe.removeAttribute("data-d11-privacy-blocked");
        iframe.removeAttribute("data-d11-privacy-original-src");
        if (placeholder && placeholder.classList.contains("d11-privacy-iframe-placeholder")) {
          placeholder.remove();
        }
      });
      return;
    ...
    document.querySelectorAll("iframe").forEach(function (iframe) {
      if (iframe.getAttribute("data-d11-privacy-blocked") === "1") {
        return;
      }
      if (iframe.closest(".d11-privacy-embed")) {
        return;
      }
      ...
      iframe.setAttribute("data-d11-privacy-original-src", src);
      iframe.setAttribute("data-d11-privacy-blocked", "1");
      iframe.setAttribute("src", "about:blank");
      iframe.style.display = "none";

      const wrapper = document.createElement("div");
      wrapper.className = "d11-privacy-iframe-placeholder";
      wrapper.innerHTML = [
        "<p>" + escapeHTML(__("To view this content you need to accept marketing cookies.", "d11")) + "</p>",
        '<button type="button" class="scc-button scc-button--secondary" data-d11-privacy-open="1">' + escapeHTML(__("Settings", "d11")) + "</button>"
      ].join("");
  ...
  function getToggleStates() {
    ...
    defaultCategories.forEach(function (category) {
      const input = banner ? banner.querySelector('[data-d11-privacy-toggle="' + category + '"]') : null;
  ...
  function setupCookiePanels() {
    document.querySelectorAll("[data-d11-privacy-cookie-row]").forEach(function (row) {
      row.addEventListener("click", function (event) {
        if (event.target.closest("input") || event.target.closest("[data-d11-privacy-cookie-toggle]")) {
          return;
        }

        const category = row.getAttribute("data-d11-privacy-cookie-row");
        const panel = document.querySelector('[data-d11-privacy-cookie-panel="' + category + '"]');
    ...
    document.querySelectorAll("[data-d11-privacy-cookie-toggle]").forEach(function (button) {
      button.addEventListener("click", function (event) {
        event.preventDefault();
        event.stopPropagation();
        const category = button.getAttribute("data-d11-privacy-cookie-toggle");
        const panel = document.querySelector('[data-d11-privacy-cookie-panel="' + category + '"]');
        const isOpen = panel && !panel.classList.contains("scc-hidden");
        setCookiePanelState(category, !isOpen);
      });
  ...
  function bindBannerActions() {
    ...
    banner.addEventListener("click", function (event) {
      const actionButton = event.target.closest("[data-d11-privacy-action]");
      ...
      const action = actionButton.getAttribute("data-d11-privacy-action");
  ...
  function bindOpenTriggers() {
    document.addEventListener("click", function (event) {
      const trigger = event.target.closest("[data-d11-privacy-open]");
      if (!trigger) {
        return;
      }
