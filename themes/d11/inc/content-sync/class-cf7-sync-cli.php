...
/**
 * Theme-owned WP-CLI synchronization for Contact Form 7 manifests.
 */
...
if ( ! class_exists( 'D11_CF7_Sync_CLI' ) ) {
	final class D11_CF7_Sync_CLI {
		const VERSION           = '0.1.1';
		const FORM_META_SLUG    = '_cf7_sync_slug';
		const TEXT_DOMAIN       = 'd11';
		...
		private static function register_cli_command() {
			...
			WP_CLI::add_command( 'content:cf7-sync', [ __CLASS__, 'handle_sync_command' ] );
		}
		 ...
		 * ## EXAMPLES
		 *
		 *     wp content:cf7-sync --dir=wp-content/themes/example-theme/inc/content-sync/cf7-forms
		 *     wp content:cf7-sync --slug=contatti --dry-run
		 */
		...
		private static function assert_cf7_available() {
			if ( ! class_exists( 'WPCF7_ContactForm' ) ) {
				\WP_CLI::error( __( 'Contact Form 7 must be installed and active before using this command.', 'd11' ) );
			}
		}
		...
		private static function resolve_forms_directory( $directory ) {
			...
			if ( '' === $directory ) {
				$directory = self::get_default_forms_directory();
			...
			if ( false === $real_path || ! is_dir( $real_path ) ) {
				\WP_CLI::error( sprintf( __( 'Manifest directory not found: %s', 'd11' ), $directory ) );
			}
		...
		private static function get_default_forms_directory() {
			return get_theme_file_path( 'inc/content-sync/cf7-forms' );
		}
		...
		private static function load_manifests( $directory, $slug_filter ) {
			...
			if ( false === $files || [] === $files ) {
				\WP_CLI::error( sprintf( __( 'No JSON manifests found in %s', 'd11' ), $directory ) );
			}
			...
			foreach ( $files as $file ) {
				...
				if ( isset( $seen[ $manifest['slug'] ] ) ) {
					\WP_CLI::error(
						sprintf(
							__( 'Duplicate slug "%1$s" found in %2$s and %3$s', 'd11' ),
							$manifest['slug'],
							$seen[ $manifest['slug'] ],
							$file
						)
			...
			if ( $slug_filter && [] === $manifests ) {
				\WP_CLI::error( sprintf( __( 'No manifest found for slug "%s"', 'd11' ), $slug_filter ) );
			}
		...
		private static function load_manifest_file( $file ) {
			...
			if ( false === $raw_json ) {
				\WP_CLI::error( sprintf( __( 'Unable to read manifest file: %s', 'd11' ), $file ) );
			}

			$data = json_decode( $raw_json, true );
			if ( JSON_ERROR_NONE !== json_last_error() ) {
				\WP_CLI::error( sprintf( __( 'Invalid JSON in %1$s: %2$s', 'd11' ), $file, json_last_error_msg() ) );
			}

			if ( ! is_array( $data ) ) {
				\WP_CLI::error( sprintf( __( 'Manifest must decode to an object: %s', 'd11' ), $file ) );
			}
			...
			if ( '' === $manifest['slug'] ) {
				\WP_CLI::error( sprintf( __( 'Missing or invalid "slug" in %s', 'd11' ), $file ) );
			}

			if ( '' === $manifest['title'] ) {
				\WP_CLI::error( sprintf( __( 'Missing "title" in %s', 'd11' ), $file ) );
			}

			if ( '' === trim( $manifest['form'] ) ) {
				\WP_CLI::error( sprintf( __( 'Missing "form" in %s', 'd11' ), $file ) );
			}
		...
		private static function normalize_mail_property( $value, $file, $property_name ) {
			if ( null === $value ) {
				if ( 'mail' === $property_name ) {
					\WP_CLI::error( sprintf( __( 'Missing "%1$s" in %2$s', 'd11' ), $property_name, $file ) );
				}
			...
			if ( ! is_array( $value ) ) {
				\WP_CLI::error( sprintf( __( '"%1$s" must be an object in %2$s', 'd11' ), $property_name, $file ) );
			}
			...
			if ( 'mail' === $property_name ) {
				...
				foreach ( $required_keys as $required_key ) {
					if ( '' === trim( $normalized[ $required_key ] ) ) {
						\WP_CLI::error( sprintf( __( 'Missing "%1$s.%2$s" in %3$s', 'd11' ), $property_name, $required_key, $file ) );
					}
				}
			}
		...
		private static function get_form_by_slug( $slug ) {
			...
			$posts = $query->posts;
			if ( count( $posts ) > 1 ) {
				\WP_CLI::error( sprintf( __( 'Multiple CF7 forms found for slug "%s"', 'd11' ), $slug ) );
			}
		...
		private static function create_form( $manifest, $dry_run ) {
			...
			if ( is_wp_error( $post_id ) ) {
				\WP_CLI::error( sprintf( __( 'Unable to create form "%1$s": %2$s', 'd11' ), $manifest['slug'], $post_id->get_error_message() ) );
			}

			$form = \WPCF7_ContactForm::get_instance( $post_id );
			if ( ! $form ) {
				\WP_CLI::error( sprintf( __( 'Unable to bootstrap CF7 form instance for "%s"', 'd11' ), $manifest['slug'] ) );
			}
		...
		private static function save_form( $form, $slug ) {
			$result = $form->save();
			if ( false === $result ) {
				\WP_CLI::error( sprintf( __( 'CF7 failed to save form "%s"', 'd11' ), $slug ) );
			}
		...
		private static function render_results( $results, $dry_run ) {
			...
			$message = sprintf(
				__( 'CF7 sync complete%s. Created: %d, Updated: %d, Unchanged: %d', 'd11' ),
				$dry_run ? ' ' . __( '(dry-run)', 'd11' ) : '',
				$counts['create'],
				$counts['update'],
				$counts['no-change']
			);
